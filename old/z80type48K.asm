...
; 2. MODIFY ULA ELECTRICAL HYSTERESIS USING OUT (C),<0|0FFH>
;    (DB 0EDH, 071H) UNDOCUMENTED INSTRUCTION:
;      ON AN NMOS CPU: OUT (C),0
;      ON A CMOS CPU: OUT (C),0FFH
; 3. WAIT FOR IT TO PROPAGATE 219 T-states ≈ 62.6 microseconds
; 4. READ ULA, check bit 6 (EAR)
;       0 - 0
;	1 - 0FFH
; 5. ENABLE INTERRUPTS
; 6. CHECK THE VALUE READ BACK IN STEP 4
;      0 - NMOS CPU
;      0FFH - CMOS CPU

	di

; exploiting ULA electrical hysteresis 

	ld 	c,0FEh      	; ULA
	DB	0EDH, 071H	; UNDOCUMENTED OUT (C),<0|0FFH> INSTRUCTION
				; WRITE 00 OR FF TO ULA
				; out bit 3 4 = 0 0, next in bit 6=0
				; out bit 3 4 = 1 1, next in bit 6=1
...
Loop:	LD	IX,0
	...
	and	040h		; bit 6 
	ld 	a,0ffh		
	jr	nz,leaveout	; leave if OUT (C) was 0FFh CMOS
	xor	a		; it was OUT (C),0 NMOS

leaveout:
        ; we dont need to restore as register 14 is not used
        ; in the Spectrum ecosystem
	ei
	ret
...
TESTU880:
	ld hl,0FFFFH
	ld bc,001FFH		; USE any port for tests...
...
MSGSIGNIN	DB	'Z80 Processor Type Detection (C) 2024 Sergey Kiselev'
		DB	0DH
MSGRUI		DB	' 2025 Ported ZX Spectrum 48K Rui Ribeiro', 0DH
MSGCRLF		DB	0DH,'$'
